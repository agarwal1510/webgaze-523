...
var siteFile = 'C:\\Users\\ASUS\\Downloads\\webgaze-523\\list.txt';
...
CDP(function(client){
  ...
  const {Network, Page, Runtime, Emulation, DOMSnapshot} = client;
  const { JSDOM } = jsdom;
      ...
      function mainIter(){
        return new Promise(function(resolve, reject){ 
          ...
          newPage().then(function(pageSuccessMsg){
            ...
            if (!hrefAdded) {
              JSDOM.fromURL(url_array[data_count]).then(dom => {
                  ...
                  for (each in atag) {
                    for (var i = 0; i < atag[each].childElementCount; i++) {
                      if (atag[each].children[i].tagName == "IMG") {
                        console.log(atag[each].href);
                        set.add(atag[each].href);
                      }
                    }
                  }
                  if (set.has(url_array[data_count]))
                    set.delete(url_array[data_count]);
                  url_array.push(...set);
